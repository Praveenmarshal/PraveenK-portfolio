...
/* ── WEBGL LIQUID & WAVE TEXT DISTORTION (FRAMER EXACT SHADER) ── */
(function initWebGLScrollTextDistortion() {
  ...
  const vertexShaderSrc = `
    ...
    void main() {
      ...
      vUv.y = 1.0 - vUv.y; // Flip Y for WebGL texture coordinate system
  ...
  const fragmentShaderSrc = `
    ...
    // 3D Simplex noise from Framer component
    ...
    void main() {
      ...
      // 1. Fluid Sine Wave Distortion (like Framer 'Wave' preset)
      float frequency = 4.6;
      float waveAmp = speed * 0.075;
      float wave = sin(uv.y * frequency * 3.14159265 + uTime * 3.2) * waveAmp;
      uv.x += wave;

      // 2. Liquid Simplex Noise Distortion (like Framer 'Liquid' preset)
      float volatility = speed * 0.095;
      float z = uTime * 0.6;
      vec2 noiseOffset = vec2(
        snoise(vec3(uv * 3.2, z)),
        snoise(vec3(uv * 3.2 + 80.0, z))
      ) * volatility;
      uv += noiseOffset;

      // 3. Glitch / Chromatic Aberration (RGB channel shift)
      float rgbShift = speed * 0.026;
  ...
  function setupWebGLHeading(headingEl) {
    const text = headingEl.textContent.trim();
    const wrapper = document.createElement('div');
    wrapper.className = 'webgl-text-distort-wrap';
    wrapper.style.cssText = 'position:relative;width:100%;display:flex;justify-content:center;align-items:center;margin-bottom:48px;overflow:visible;z-index:2;';

    const canvas = document.createElement('canvas');
    canvas.className = 'webgl-text-distort-canvas';
    canvas.style.cssText = 'width:100%;max-width:1100px;display:block;margin:0 auto;';
    wrapper.appendChild(canvas);

    headingEl.style.display = 'none';
    headingEl.parentNode.insertBefore(wrapper, headingEl);
    ...
    const uScrollSpeedLoc = gl.getUniformLocation(prog, 'uScrollSpeed');
    ...
    function renderTextToTexture(w, h) {
    ...
    function onScroll() {
      ...
      lastScroll = current;
      targetSpeed = Math.min(delta / 25, 1.4); // Sensitivity
    }
    ...
    function renderLoop() {
      time += 0.016;
      scrollSpeed = lerp(scrollSpeed, targetSpeed, 0.09);
      targetSpeed = lerp(targetSpeed, 0, 0.075); // Smooth decay

      const rect = wrapper.getBoundingClientRect();
      const inView = rect.bottom > -100 && rect.top < (window.innerHeight || document.documentElement.clientHeight) + 100;

      if (inView || scrollSpeed > 0.005) {
        gl.useProgram(prog);
        gl.uniform1f(uTimeLoc, time);
        gl.uniform1f(uScrollSpeedLoc, scrollSpeed);
  ...
  if (document.fonts && document.fonts.ready) {
    document.fonts.ready.then(function() {
      headings.forEach(setupWebGLHeading);
    });
  } else {
    headings.forEach(setupWebGLHeading);
  }
